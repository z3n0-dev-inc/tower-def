...
const PF = {
  ...
  async login(username, password, silent = false) {
    ...
    if (r.code === 200) {
      ...
      // ── Ban check — block login if banned ────
      try {
        const banRes = await fetch(`${this.SERVER_URL}/auth/checkBan`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ playFabId: this.playFabId }),
        });
        const banData = await banRes.json();
        if (banData.banned) {
          this.sessionTicket = null;
          this.playFabId     = null;
          return { ok: false, banned: true, reason: banData.reason, durationText: banData.durationText, permanent: banData.permanent };
        }
      } catch { /* server offline — allow login */ }
  ...
  async staffCall(action, params) {
    try {
      const r = await fetch(`${this.SERVER_URL}/staff/${action}`, {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ callerSession:this.sessionTicket, ...params }),
      });
      return await r.json();
