/* ═══════════════════════════════════════════════
   towers.js — All tower definitions & logic
   ═══════════════════════════════════════════════ */
...
const TOWER_DEFS = [
  // ── BASIC ──────────────────────────────────────
  {
    ...
    rarity: 'basic',
    cost: 75,
    shopCost: 0, // Free / starter
    desc: 'Standard soldier. Good all-rounder.',
    color: '#3498db',
    range: 120,
  ...
  },
  {
    ...
    cost: 60,
    shopCost: 0,
    desc: 'Long range, low damage. Great for early waves.',
    color: '#27ae60',
    range: 160,
    fireRate: 1.8,
    damage: 10,
  ...
  },
  {
    ...
    cost: 120,
    shopCost: 250,
    desc: 'Very long range, high damage, slow fire.',
    color: '#8e44ad',
    range: 260,
    fireRate: 0.4,
    damage: 80,
  ...
  // ── ADVANCED ───────────────────────────────────
  {
    ...
    cost: 200,
    shopCost: 500,
    desc: 'Explosive splash damage. Destroys groups.',
    color: '#e67e22',
    range: 130,
    fireRate: 0.6,
    damage: 60,
  ...
  },
  {
    ...
    cost: 160,
    shopCost: 450,
    desc: 'Slows enemies. Low damage but vital support.',
    color: '#3498db',
    range: 110,
    fireRate: 0.8,
    damage: 10,
    ...
    bulletSize: 8,
    splash: 50,
    slow: 0.45,     // multiplier applied to speed
    slowDuration: 2.0,
    pierce: 0,
    maxUpgrade: 3,
    upgrades: [
  ...
  },
  {
    ...
    cost: 180,
    shopCost: 500,
    desc: 'Continuous flame. Burns enemies over time.',
    color: '#e74c3c',
    range: 90,
    fireRate: 8,    // rapid small hits
    damage: 8,
    bulletSpeed: 180,
    bulletColor: '#ff6b35',
    bulletSize: 6,
  ...
  // ── SPECIAL ────────────────────────────────────
  {
    ...
    bulletSize: 3,
    splash: 0,
    chain: 4,       // chain to N nearby enemies
    slow: 0.2,
    pierce: 0,
    maxUpgrade: 3,
    upgrades: [
  ...
  // ── LEGENDARY ──────────────────────────────────
  {
    ...
    cost: 800,
    shopCost: 4000,
    desc: 'Devastating orbital strike. Extremely rare.',
    color: '#f1c40f',
    range: 200,
    fireRate: 0.25,
    damage: 400,
  ...
  },
  {
    ...
    cost: 700,
    shopCost: 3500,
    desc: 'Invisible. Can target invisible enemies. Insta-kills weak foes.',
    color: '#8e44ad',
    range: 300,
    fireRate: 0.8,
    damage: 250,
  ...
  // ── OWNER-ONLY ─────────────────────────────────
  {
    ...
    cost: 0,
    shopCost: 0,
    desc: 'Owner exclusive. Buffs all nearby towers by 50%.',
    color: '#1a1a2e',
    range: 220,
    fireRate: 2.0,
    damage: 300,
    ...
    slow: 0.3,
    armorPierce: true,
    aura: true,      // boosts nearby towers
    auraBonus: 0.5,
    pierce: 0,
    maxUpgrade: 0,
    upgrades: [],
  ...
  },
  {
    ...
    cost: 0,
    shopCost: 0,
    desc: 'Owner exclusive. Electrifying area denial.',
    color: '#00ffaa',
    range: 180,
    fireRate: 5.0,
    damage: 150,
  ...
  },
  {
    ...
    cost: 0,
    shopCost: 0,
    desc: 'Owner exclusive. Pulls enemies back on kill.',
    color: '#160041',
    range: 250,
    fireRate: 1.5,
    damage: 500,
...
class Tower {
  constructor(def, tileX, tileY, tileSize) {
    ...
    this.angle = 0;
    this.selected = false;
    this.sellValue = Math.floor(def.cost * 0.6);
  ...
  getSellValue() {
    let v = Math.floor(this.def.cost * 0.6);
    for (let i = 0; i < this.level; i++) {
      v += Math.floor(this.def.upgrades[i].cost * 0.6);
    }
    return v;
  }
  ...
  _pickTarget(enemies) {
    const inRange = enemies.filter(e => {
      ...
      return Math.sqrt(dx*dx + dy*dy) <= this.range && !e.dead;
    });
  ...
  _fire(target, allEnemies) {
    ...
    this.bullets.push(new Bullet({
      ...
      splash: this.splash,
      slow:   this.slow,
      slowDur: def.slowDuration || 1.5,
      burn:   this.burn,
      chain:  this.chain,
      pierce: this.def.pierce || 0,
      armorPierce: def.armorPierce || false,
      allEnemies,
    }));
  }
  ...
  draw(ctx) {
    ...
    // Tile base
    ctx.fillStyle = this.selected ? 'rgba(241,196,15,0.25)' : 'rgba(0,0,0,0.4)';
    ...
    ctx.fillStyle = this.def.color + 'aa';
    ctx.fill();
    ...
    if (this.level > 0) {
      for (let i = 0; i < this.level; i++) {
        ...
        ctx.beginPath();
        ctx.arc(this.x - (this.level-1)*4 + i*8, this.y + s*0.32, 3, 0, Math.PI*2);
        ctx.fill();
      }
    }
    ...
    if (this.selected) {
      ...
      ctx.beginPath();
      ctx.arc(this.x, this.y, s*0.42, 0, Math.PI*2);
      ctx.strokeStyle = '#f1c40f';
      ctx.lineWidth = 2;
      ctx.stroke();
    }

    // Aura for buff towers
    if (this.def.aura) {
      this.auraAlpha = (Math.sin(Date.now() * 0.003) + 1) * 0.15;
      ctx.beginPath();
